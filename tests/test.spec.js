...
test("that each index section has some content", async ({ page }) => {
  ...
  // Mathematics
  await expect(
    page.getByText("algebraic groups over finite fields")
  ).toBeVisible();
  await expect(
    page.getByText("unipotent groups by group extensions")
  ).toBeVisible();

  // Contact
  await expect(
    page.getByText("gmail: seth.j.rothschild", { exact: true })
  ).toBeVisible();
  await expect(
    page.getByText("dell: seth.rothschild", { exact: true })
  ).toBeVisible();
...
test("that each programming page section has some content", async ({
  page,
}) => {
  await page.goto("/programming");
